...
DR.Game = {
  init(setupTeams, timerMinutes) {
    const state = {
      ...
      timerRunning: false,
      sprintActive: false,
      phase: 'awaiting_roll', // awaiting_roll | landed | question | ended
      lastRoll: null,
  ...
  grantFragment(state, team, key) {
    ...
    if (used >= team.backpackCap) {
      const gained = this.changeMerit(state, team, 2);
      this.log(state, `${team.icon}${team.name} 行囊已满,残页换成了 ${gained} 点功德。`);
      return { converted: true, meritGained: gained };
    }
    team.backpack[realKey]++;
    const def = DR.PARAMITAS.find(p => p.key === realKey);
  ...
  moveAndResolve(state) {
    ...
    const result = { team, station, firstTime, canTrade: station.type === 'site' || station.type === 'final' };
  ...
  buyFragment(state, key) {
    const team = this.activeTeam(state);
    if (team.merit < DR.CONFIG.buyCost) return { ok: false, reason: '功德不足' };
    if (backpackCount(team) >= team.backpackCap) return { ok: false, reason: '行囊已满' };
    team.merit -= DR.CONFIG.buyCost;
    state.bank += DR.CONFIG.buyCost;
    team.backpack[key]++;
    const def = DR.PARAMITAS.find(p => p.key === key);
    this.log(state, `${team.icon}${team.name} 以功德换得《${def.name}》残页 ${def.icon}`);
    return { ok: true };
  },
  ...
  swapFragment(state, giveKey, takeKey) {
    ...
    const g = DR.PARAMITAS.find(p => p.key === giveKey);
    const t = DR.PARAMITAS.find(p => p.key === takeKey);
  ...
  sellFragments(state, keys) {
    ...
    let value = keys.length * DR.CONFIG.sellValue;
  ...
  computeResults(state) {
    const rows = state.teams.map(team => {
      ...
      const fragValue = fragCount * DR.CONFIG.sellValue + (fullSet ? DR.CONFIG.fullSetBonus : 0);
      return {
        team,
        fragCount,
        fullSet,
